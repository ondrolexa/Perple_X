...
99981 format (' xxx  cannot satisfy the working set constraints to the',
     *       ' accuracy requested.')
      ...
      subroutine nlpopt (n,nclin,ncnln,lda,ldcju,ldr,a,bl,bu,confun,
     *                  objfun,iter,istate,c,cjacu,clamda,objf,gradu,r,
     *                  x,iw,leniw,w,lenw,iuser,user,ifail)

c     solves the nonlinear program
c
c            minimize                   f(x)
c
c                                    (    x  )
...
c     now check the gradients at a feasible x.
...
c     .. local arrays ..
      ...
      character*80      rec(3)
c     .. external subroutines ..
      external          dcopy, e04mfx, f06dff, x04bay
c     .. intrinsic functions ..
...
c     .. local arrays ..
      ...
      character*80      rec(4)
c     .. external subroutines ..
      external          dcopy, e04ucq, f06dff, x04baf, x04bay
c     .. intrinsic functions ..
...
c  if  r is nearly singular then the  library routine  f02wuf can be
...
c     .. scalar arguments ..this is adivb
...
c  which  case the value  sign( t )*flmax  is returned
c
c  c and s  can be reconstructed from the tangent,  t,  by a call to the
c  basic linear algebra routine f06bcf.
