...
c                               use ichk to avoid multiple messages
     ...
     *          iam.eq.5.or.iam.eq.6) call warn (13,tot,j,name)
...
c                                 special case (1 component solution). 
         if (ksmod(i).eq.39.and.ndim(1,i).eq.1) cycle
...
1040  format (/,'Simplex fraction ranges for prismatic model: ',a)
...
c         gtot = gtot + slvmo(i) * (gso(i) + rt*dlog(yf(ins(i))))
      end do 
c                                 bulk fluid composition 
      totm = 0d0
      err = dabs(err)*1d1
...
c                                check on charge imbalance
         ...
         write (*,*) 'err, posox+negox0',err,posox+negox
         ...
         write (*,*) 'err, posox+negox1',err,posox+negox
...
c                                 a variation thereof, this is intended 
c                                 to kill species that contain an absent
c                                 component, but will also kill net redox 
c                                 neutral species (Fe0+Fe3+ etc). 
            if (cblk(j).eq.0d0.and..not.lopt(36)) then 
               if (aqcp(j,i).ne.0d0) then 
                  kill = .true.
                  exit
               end if 
            end if 
c                                 species that includes a degnerate
c                                 component. 
            dg = dg + dn * mu(j)
